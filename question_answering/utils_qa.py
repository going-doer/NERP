...
def transform_n2b_listqa(nbest_path, output_path):
    ...
    for qid in qidDict:
        ...
        u"ideal_answer":["Dummy"],
        # u"exact_answer":[[ans[0]] for ans in finalSorted[:5]],
        u"exact_answer":[[ans[0]] for ans in finalSorted[:final_idx]],
        # I think enough?
        }
        entryList.append(entry)
        
        entryWithProb={u"type":"list", 
        u"id":qid, # must be 24 char
        u"ideal_answer":["Dummy"],
        # u"exact_answer":[ans for ans in finalSorted[:20]],
        u"exact_answer":[ans for ans in finalSorted[:final_idx]],
        }
        entryListWithProb.append(entryWithProb)
    finalformat={u'questions':entryList}
    finalformatWithProb={u'questions':entryListWithProb}
...
def eval_bioasq_standard(task_num, outfile, golden, cwd):
    ...
    # java -Xmx10G -cp $CLASSPATH:./flat/BioASQEvaluation/dist/BioASQEvaluation.jar 
    # evaluation.EvaluatorTask1b -phaseB -e 5 golden_file.json system_response.json
